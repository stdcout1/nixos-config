...
in {
  ...
  config = mkIf cfg.enable {
	  ...
	  cmd dragon %{{
	    ${pkgs.xdragon}/bin/dragon $f
	  }}

	  cmd dragall %{{
	    ${pkgs.xdragon}/bin/dragon --all $fx
	  }}
	      ...
	      # pdf files
	      application/pdf|application/cb7|application/cba|application/cbr|application/cbt|application/cbz)
